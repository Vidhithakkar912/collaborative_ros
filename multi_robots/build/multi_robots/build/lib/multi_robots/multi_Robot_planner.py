...
class LLMCommandResolver:
    ...
    _SYSTEM_PROMPT = """
Convert robot command into JSON.
...
"sr1 go to room1"
{{"commands":[{{"mode":"waypoint_chain","robot":"sr1","goals":["room1"]}}]}}


"go to room1"
{{"commands":[{{"mode":"auto_location","goals":["room1"]}}]}} 


"sr1 go to box object"
{{"commands":[{{"mode":"named_robot_object","robot":"sr1"}}]}}
...
"sr1 move left"
{{"commands":[{{"mode":"movement","robot":"sr1","direction":"left"}}]}}

"stop sr1"
{{"commands":[{{"mode":"stop","robot":"sr1"}}]}}

IMPORTANT RULES:

1. If robot name is explicitly mentioned (example: sr1, sr2),
   NEVER use auto_location.
   ALWAYS use waypoint_chain with that robot.

2. ONLY use auto_location when NO robot name is mentioned.

3. waypoint_chain = explicit robot
4. auto_location = nearest robot selection
...
"""
    ...
    def build_prompt(self, user_text: str, status: dict) -> str:
        system = self._SYSTEM_PROMPT.format(
            robots=", ".join(self._robots),
            locations=", ".join(self._locations),
            status=json.dumps(status),
        )
        return f'{system}\nInput: "{user_text}"\nJSON:'

    def resolve(self, user_text: str, llm,  status: dict) -> dict | None:
        prompt = self.build_prompt(user_text, status)
        

        try:
            self._logger.info("[LLM] Running inference ...")
            inf_time1= time.time()
            result = llm(
                prompt,
                max_tokens=200,
            ...
            inf_time2=time.time()
            total_inf=inf_time2-inf_time1
            self._logger.info(f"inf time is :{total_inf}")
            raw = result["choices"][0]["text"].strip()
            self._logger.info(f"[LLM] Raw output: {raw}")
        except Exception as e:
            self._logger.error(f"[LLM] Inference error: {e}")
            return None
        extract_json_time1=time.time()
        parsed = self._extract_json(raw)
        extract_json_time2=time.time()
        total_extraction_time=extract_json_time2-extract_json_time1
        self._logger.info(f"extract json parse time is :{total_extraction_time}")
        if parsed:
            self._logger.info(f"[LLM] Resolved: {json.dumps(parsed)}")
        else:
            self._logger.error("[LLM] Could not extract valid JSON from output")
        return parsed
    ...
    def _extract_json(self, raw: str) -> dict | None:
        ...
        import regex
        candidates = []
        for m in regex.finditer(r'\{(?:[^{}]|(?0))*\}', raw):
            try:
                obj = json.loads(m.group(0))
                candidates.append((len(m.group(0)), obj))
...
class MultiRobotPlanner(Node):
    ...
    """
    _ARRIVAL_THRESH = 0.3    # metres
    _GOAL_TIMEOUT   = 60     # seconds
    ...
    def _mode_waypoint_chain(self, robot: str, goals: list):
        if robot not in self._robots:
            self._feedback(f" robot is not known: {robot}"); return
    ...
    def _process_chat(self, text: str):
        ...
        self._feedback("planner","processing")
        parsed = self._llm_resolver.resolve(
            user_text, self._llm, self._status)

        if not parsed:
            self.get_logger().error("[LLM] Failed to resolve command")
            return
