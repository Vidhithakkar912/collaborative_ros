...
class ChatGUI(Node):
    ...
    def __init__(self):
        ...
        self.create_timer(3.0, self._scan_image_topics)
    ...
    def _build_entry_bar(self):
        bar = ctk.CTkFrame(self.window, fg_color=BG_PANEL,
                           corner_radius=12, height=62)
        ...
        self._entry = ctk.CTkEntry(
            bar,
            font=("Courier New", 13),
            placeholder_text="  e.g.  sr1 go to room1  |  go to box object  |  stop sr2",
            height=36, corner_radius=10,
            fg_color=BG_ENTRY, border_color="#cccccc",
            text_color=TEXT_LIGHT)
        self._entry.grid(row=0, column=0, padx=(10, 6), pady=12, sticky="ew")
        self._entry.bind("<Return>", self._send)

        ctk.CTkButton(bar, text="SEND ▶", command=self._send,
                      font=("Courier New", 12, "bold"),
                      fg_color=ACCENT, hover_color="#c73652",
                      corner_radius=10, width=96, height=36
                      ).grid(row=0, column=1, padx=(0, 10), pady=12)

    # ──────────────────────────────────────────────────────────────────
    #  Camera panel  (middle column)
    # ──────────────────────────────────────────────────────────────────
    ...
    # ──────────────────────────────────────────────────────────────────
    #  Image topic scanner  (runs every 3 s via ROS timer)
    # ──────────────────────────────────────────────────────────────────
    ...
    def _build_status_panel(self):
        ...
        # self._status_inner = ctk.CTkScrollableFrame(
        #     panel, fg_color=BG_PANEL, corner_radius=0)
        # self._status_inner.pack(fill="both", expand=True, padx=6, pady=6)
    ...
    def _on_robot_status(self, msg: String):
        ...
        #         ry        = info.get("y", 0.0)
        #         goal      = info.get("goal", "—")
        #         dot_color = (STATUS_IDLE  if status == "idle"  else
        #                      STATUS_ERROR if status == "error" else
        #                      STATUS_BUSY)

        #         if robot not in self._robot_frames:
                    # card = ctk.CTkFrame(self._status_inner,
                    #                     fg_color="#dce8f5", corner_radius=8)
                    # card.pack(fill="x", pady=4, padx=2)
                    ...
                    # dot.grid(row=0, column=0, padx=(8, 4), pady=(8, 2))

                    # ctk.CTkLabel(card, text=robot.upper(),
                    #              font=("Courier New", 12, "bold"),
                    #              text_color=SENDER_COLORS.get(robot, TEXT_DIM),
                    #              anchor="w"
                    #              ).grid(row=0, column=1, sticky="w", pady=(8, 2))

                    # info_lbl = ctk.CTkLabel(card, text="",
                    #                         font=("Courier New", 10),
                    #                         text_color=TEXT_DIM, anchor="w")
                    # info_lbl.grid(row=1, column=0, columnspan=2,
                    #               sticky="w", padx=10, pady=(0, 6))

                    # self._robot_frames[robot] = card
                    # self._robot_labels[robot] = {"dot": dot, "info": info_lbl}

                # lbls = self._robot_labels[robot]
                # lbls["dot"].configure(text_color=dot_color)
                # lbls["info"].configure(
