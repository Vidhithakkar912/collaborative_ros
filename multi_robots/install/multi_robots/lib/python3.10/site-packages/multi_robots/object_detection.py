...
# def detect_rectangles(model,
#                       image: np.ndarray,
#                       min_conf: float = 0.25) -> list[list[int]]:
...
class PoseEstimation(Node):
    ...
    def _rgb_callback(self, msg: Image):
        ...
        try:
             #new_bboxes = detect_rectangles(self.model,cv_img)
              new_bboxes = detect_rectangles(cv_img, min_area=self.min_bbox_area_)
    ...
    def _depth_img_callback(self, msg: Image):
            ...
            scale_x = depth_w / self.rgb_w
            scale_y = depth_h / self.rgb_h

            scaled_bboxes = []

            for xmin, xmax, ymin, ymax in bboxes_snapshot:

                scaled_bboxes.append([
                    max(0, min(depth_w - 1, int(xmin * scale_x))),
                    max(0, min(depth_w - 1, int(xmax * scale_x))),
                    max(0, min(depth_h - 1, int(ymin * scale_y))),
                    max(0, min(depth_h - 1, int(ymax * scale_y))),
                ])
            ...
            for idx, (xmin, xmax, ymin, ymax) in enumerate(scaled_bboxes):
                ...
                self.get_logger().info(f"cx_pix cypix{cx_pix}{cy_pix}")
                window = 5
