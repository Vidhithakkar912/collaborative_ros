...
class GolfBallPipeline(Node):
    ...
    def get_robot_pose(self):
        #self.get_logger().info("goto_get_robot_pose_callback")
        try:
            ...
            #         f"""
            #     Transform:
            #     parent frame : {t.header.frame_id}
            #     child frame  : {t.child_frame_id}

            #     Translation:
            #     x = {t.transform.translation.x:.3f}
            #     y = {t.transform.translation.y:.3f}
            #     z = {t.transform.translation.z:.3f}

            #     Rotation (quaternion):
            #     x = {t.transform.rotation.x:.3f}
            #     y = {t.transform.rotation.y:.3f}
            #     z = {t.transform.rotation.z:.3f}
            #     w = {t.transform.rotation.w:.3f}
            #     """
            #     )
            ...
            self.get_logger().warn(
                f'could not transform {self.map_frame} to 'f'{self.robot_base_frame}: {ex}')
    ...
    def golf_ball_pose_callback(self, msg):
                #self.get_logger().info("golf ball pose callback is called: ")
                if self.robot_busy:
    ...
    def send_goal(self, pose, keep_orientation=False, add_offset=True):
        ...
        self.goal_target_x = goal_pose.pose.position.x
        self.goal_target_y = goal_pose.pose.position.y
    ...
    #             self.robot_busy = False
        ...
        Thread(target=self._wait_for_arrival, args=(pose,), daemon=True).start()

    def _wait_for_arrival(self, original_pose, timeout=60.0):
        start = time.time()
        while time.time() - start < timeout:
            ...
            dx = t.transform.translation.x - self.goal_target_x
            dy = t.transform.translation.y - self.goal_target_y
